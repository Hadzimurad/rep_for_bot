...
# Создаем клавиатуру с 6 кнопками
...
async def handle_appointment(message: types.Message, state: FSMContext):
    response = (
        ...
        "Если нет то нажмите кнопку Сброс"
    )
    await message.answer(response, parse_mode="HTML")
...
# Обработка кнопки "🎁 Акции и скидки"
@dp.message(lambda message: message.text == "🎁 Акции и скидки")
async def handle_promotions(message: types.Message):
    response = (
        "🎁 <b>Актуальные акции и скидки — Русфера</b>\n\n"
        
        "🔥 <b>Новым клиентам:</b>\n"
        "• Скидка 20% на первый ремонт\n"
        "• Диагностика бесплатно\n"
        "• Выезд специалиста без доп. платы\n\n"
        
        "👴 <b>Пенсионеры и студенты:</b>\n"
        "• Постоянная скидка 15%\n"
        "(при предъявлении удостоверения)\n\n"
        
        "👥 <b>Программа лояльности «Постоянный клиент»:</b>\n"
        "• Приведи друга — получи 10% скидку\n"
        "• Кэшбэк 5% на каждый ремонт\n"
        "• Скидка 10% на повторный ремонт\n\n"
        
        "🏢 <b>Для организаций и ИП:</b>\n"
        "• Договор на годовое обслуживание\n"
        "• Специальные цены\n"
        "• Выезд специалиста на место\n"
        "• Приоритет в очереди ремонтов\n\n"
        
        "📅 <b>Сезонные акции:</b>\n"
        "• «Осенняя чистка» — чистка от пыли 1000 руб.\n"
        "• «Новый год» — 25% скидка на ремонт ПК\n"
        "• «1 сентября» — 20% скидка на ремонт печатающей техники\n\n"
        
        "📱 <b>Спецпредложение для клиентов бота:</b>\n"
        "Покажите этот чат и получите:\n"
        "✅ Дополнительную скидку 5%\n"
        "✅ Бесплатный апгрейд диагностики\n\n"
        
        f"ℹ️ Подробности: {COMPANY_WEBSITE}/promo\n"
        "Акции действуют до 31.12.2025"
    )
    await message.answer(response, parse_mode="HTML")
